summary.ciTarFit <- function(object, digits = 3, ...)
{
  ...
  dia <- data.frame(matrix(NA, nrow=14, ncol=2)); colnames(dia) <- c("item", mn)
  dia[1, ] <-c("lag",       w$lag)
  ...
  dia[12, ]<-c("LB test(4)", Box.test(resid(w$CI), lag=4, type="Ljung")$p.value)
  dia[13, ]<-c("LB test(8)", Box.test(resid(w$CI), lag=8, type="Ljung")$p.value)
  dia[14, ]<-c("LB test(12)",Box.test(resid(w$CI), lag=12,type="Ljung")$p.value)
  ...
  out  <-rbind(cbind(model=mn, reg="LR", c.LR), cbind(model=mn, reg="CI", c.CI))
  colnames(out) <- c("model", "reg", "estimate","st.error","t.value","p.value")
