...
function(w, digits=3, ... ) 
{
    ...
    Hypo[1] <- h2 <- paste(names(coef(w$ecm.x))[(tot-1)], names(coef(w$ecm.x))[tot], sep="=")       
    ...
    result <-list(H1ex=H1ex, H1ey=H1ey, H2xx=H2xx, H2yx=H2yx, H2xy=H2xy, H2yy=H2yy)

    cx <- cy <- cum.x <- cum.y <- matrix(0, nrow=1, ncol=gg*2*L+3)                
    if (w$split) {
        for (j in 1:L) {
            cum.x[1+j] <- 1; cum.x[1+j+L] <- -1
            cx <- matrix(0, nrow=1, ncol=4*L+3); cx[1+j] <- 1; cx[1+j+L] <- -1
            Hypo[3+j] <-paste(names(coef(w$ecm.x))[1+j], "=", names(coef(w$ecm.x))[1+j+L])
            H3xx <- lht(w$ecm.x, hypothesis.matrix=cx, rhs=0)
            H3yx <- lht(w$ecm.y, hypothesis.matrix=cx, rhs=0)
            F.sta.x[3+j] <-round(H3xx[,5][2], dd); F.sta.y[3+j] <-round(H3yx[,5][2], dd)
            P.val.x[3+j] <-round(H3xx[,6][2], dd); P.val.y[3+j] <-round(H3yx[,6][2], dd)
    
            cum.y[1+j+2*L] <- 1; cum.y[1+j+3*L] <- -1
            cy <- matrix(0, nrow=1, ncol=4*L+3); cy[1+j+2*L] <- 1; cy[1+j+3*L] <- -1
            Hypo[3+j+L] <- paste(names(coef(w$ecm.x))[1+j+2*L], "=", names(coef(w$ecm.x))[1+j+3*L]) 
            H3xy <- lht(w$ecm.x, hypothesis.matrix=cy, rhs=0 )
            H3yy <- lht(w$ecm.y, hypothesis.matrix=cy, rhs=0 )
            F.sta.x[3+j+L] <-round(H3xy[,5][2], dd); F.sta.y[3+j+L] <-round(H3yy[,5][2], dd)
            P.val.x[3+j+L] <-round(H3xy[,6][2], dd); P.val.y[3+j+L] <-round(H3yy[,6][2], dd)
        }    
        ...
        Hypo[3+2*L+1] <-paste("Cumulative positive", w$name.x, "=","Cumulative negative", w$name.x)
        Hypo[3+2*L+2] <-paste("Cumulative positive", w$name.y, "=","Cumulative negative", w$name.y)
        F.sta.x[3+2*L+1]<-round(H4xx[,5][2], dd); F.sta.y[3+2*L+1]<-round(H4yx[,5][2], dd)
        P.val.x[3+2*L+1]<-round(H4xx[,6][2], dd); P.val.y[3+2*L+1]<-round(H4yx[,6][2], dd)
        F.sta.x[3+2*L+2]<-round(H4xy[,5][2], dd); F.sta.y[3+2*L+2]<-round(H4yy[,5][2], dd)
        P.val.x[3+2*L+2]<-round(H4xy[,6][2], dd); P.val.y[3+2*L+2]<-round(H4yy[,6][2], dd)
        
        result$H3xx <- H3xx; result$H3yx <- H3yx; result$H3xy <- H3xy; result$H3yy <- H3yy
        result$H4xx <- H4xx; result$H4yx <- H4yx; result$H4xy <- H4xy; result$H4yy <- H4yy
        brief[4:(3+L*2+2)] <- c(rep("H3: Distributed lag asymmetry|", 2*L), 
