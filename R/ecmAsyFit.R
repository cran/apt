ecmAsyFit <-
function(y, x, lag=1, split=TRUE, 
    model=c("linear", "tar", "mtar"), thresh,... ) 
{
    ...
    db <- db[, c(((1+lag)*0 + 1):((1+lag)*1), ((1+lag)*2 + 1):((1+lag)*3),
                 ((1+lag)*1 + 1):((1+lag)*2), ((1+lag)*3 + 1):((1+lag)*4 + 2)) ]

    DepVar.x <- da[,1]; DepVar.y <- da[,lag+2]
    if (split) { X. <- db[, c(-1, -(lag+2), -(2*lag+3), -(3*lag+4))]
        } else { X. <- da[, c(-1, -(lag+2))] }
    ecm.x <- lm(DepVar.x ~ 1 + X.); ecm.y <- lm(DepVar.y ~ 1 + X.)
    
    result <- list(y=y, x=x, lag=lag, split=split, model=model, 
        IndVar=X., name.x=name.x, name.y=name.y, ecm.y=ecm.y, ecm.x=ecm.x)
